document.addEventListener("DOMContentLoaded", () => {
    ...
    /* スライダー（swiper）関連 */
    const rootEl = document.querySelector('.thumbnails');
    // 連動用（サムネイルスライダー）画像群を加工して生成：画像群の中身（pタグたち）を必須要素の .swiper-wrapperでラップ
    const imgBox = rootEl.querySelector('.imgBoxes');
    imgBox.innerHTML = `<div class="swiper-wrapper">${imgBox.innerHTML}</div>`;

    // サムネイル（スライダー）の子要素
    const imgBoxes = rootEl.querySelectorAll('.imgBoxes .swiper-wrapper p');
    // メイン（スライダー）の必須要素
    const swiperWrapper = rootEl.querySelector('#forView-SwiperWrapper');
    // メインの機能全体をラップするグローバル要素
    const forBase = rootEl.querySelector('.forBase');
    ...
    while (conuter < imgBoxes.length) {
         ...
         */
        swiperWrapper.insertAdjacentHTML('beforeend', `<div class="swiper-slide"><div class="swiperImges"><span style="color:#fff;">${conuter + 1}/${imgBoxes.length}</span><p>${imgBoxes[conuter].innerHTML}</p></div></div>`);
        conuter++;
    }

    // メイン（スライダー）の仕様を指定
    const mainSlider = new Swiper('.thumbnails .swiper', {
        loop: false, // true;にすると、最初[0]と最後[13]の境目もスライドできるようになる
        grabCursor: true,
    });

    // サムネイル（スライダー）の仕様を指定
    const Thumbnail = new Swiper('.thumbnails .imgBoxes', {
    ...
    imgBoxes.forEach(imgBox => {
        ...
        imgBox.addEventListener('click', () => {
            document.querySelectorAll('.AnimateSliderWrapper, .SwiperWrapper').forEach(noViewSwiper => {
                noViewSwiper.classList.add('zIndex');
            });
    ...
    const resetBtn = rootEl.querySelector('.forBase button');
    resetBtn.addEventListener('click', () => {
        forBase.classList.remove('goView');
        document.querySelectorAll('.AnimateSliderWrapper, .SwiperWrapper').forEach(noViewSwiper => {
            noViewSwiper.classList.remove('zIndex');
        });
    });
});